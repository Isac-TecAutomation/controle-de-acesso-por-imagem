...
class Database:
    ...
    def get_device_data(self, device_data, table):
        """
        ...
        try:
            keys_string = ', '.join(device_data.keys())

            # Criar a cláusula WHERE usando placeholders para parâmetros
            where_conditions = ' AND '.join([f"{key}=%s" 
                                             for key in device_data.keys()])

            # Construir o comando SQL
            command = f"SELECT {keys_string} FROM {table} WHERE {where_conditions}"

            # Extrair os valores de device_data para passar como parâmetros
            params = tuple(device_data.values())

            # Executar o comando SQL
            cursor.execute(command, params)

            # Recuperar um único resultado
...
class Commands:
    ...
    def face_verify_database_image(self, trust_threshold):
        """
        ...
        # Carrega os encodings e dados dos usuários do banco de dados
        data_users = self.load_data_users()  # Supondo que esta função retorna uma lista de tuplas (encoding, user_data)

        # Verifica se há dados de usuários carregados
        if not data_users:
            return {"message": "Nenhum dado de usuário encontrado no banco de dados.", "success": False}, frame

        # Separa os encodings e os dados dos usuários
        encoding_data = [encoding for encoding, _ in data_users]
        user_data = [data for _, data in data_users]
        ...
        for face_encoding, (top, right, bottom, left) in zip(face_encodings, face_locations):
            # Encontra a melhor correspondência no banco de dados para o rosto detectado
            best_match, best_distance = self.find_best_match_database(face_encoding, encoding_data, user_data)
    ...
    def find_best_match_database(self, face_encoding, encoding_data, user_data):
        """
        Encontra a melhor correspondência para o encoding do rosto no banco de dados.

        :param face_encoding: array - Encoding do rosto detectado.
        :param encoding_data: list - Lista de encodings do banco de dados.
        :param user_data: list - Lista de dados dos usuários associados aos encodings.
        ...
        # Itera por cada entrada no banco de dados
        for encoding, data in zip(encoding_data, user_data):
            try:
                # Calcula a distância entre o encoding detectado e o do banco de dados
                distance = fr.face_distance([encoding], face_encoding)[0]
                ...
                print(f"Erro ao processar encoding do banco de dados: {str(e)}")
                continue
    ...
    def authenticate_device(self, device_data_json):
        ...
        device_data, error_message = db.get_device_data(device_data_json, self.device_table)
